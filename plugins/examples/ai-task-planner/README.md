...
一个智能的 FlashNote 插件,帮助你使用 AI 拆解复杂任务为可执行的待办事项。**支持学习用户偏好,实现个性化规划。**
...
- 💾 **一键创建**:批量创建待办事项到 FlashNote
- 🎨 **Material-UI 设计**:使用 Material-UI 组件,与主应用风格完全一致
- 🌈 **主题同步**:自动同步 FlashNote 的浅色/深色主题
...
插件界面使用 **Material-UI** 组件库,与 FlashNote 主应用风格完全一致:
...
2. 启动 FlashNote 开发环境：`npm run electron-dev`
3. 在插件商店刷新并安装插件
4. 修改代码后点击"重载插件"
